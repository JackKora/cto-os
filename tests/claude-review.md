...
Applied through the shared procedure at `meta/skill-reviewer.md`, its thin Claude/Codex adapters, and the pre-commit hook (`hooks/pre-commit`).
...
**What:** `SKILL.md`, `CLAUDE.md`, `AGENTS.md`, and `docs/*.md` files do not describe features as "runs," "is configured," "gates," etc. when the referenced code, script, workflow, or file doesn't exist in the repo.
...
**What:** No user-specific values in `SKILL.md`, `CLAUDE.md`, `AGENTS.md`, or `docs/*.md` files. User-specific means: real people's names that aren't framed as placeholders, specific company names, concrete dated events tied to the user's history, stated personal goals.
...
## 8. Root project-instruction layout matches reality

**What:** The "Layout" tree in the canonical root `CLAUDE.md` lists only directories that exist in the repo, and `AGENTS.md` resolves to that canonical file.

**How:** Parse the tree in `CLAUDE.md`. For each named directory or file, check it exists. Verify `AGENTS.md` is a symlink to `CLAUDE.md`.
