...
- **`zip_data.py`** — zips `$CTO_OS_DATA` (including `.git/`) to a timestamped archive under `$CTO_OS_DATA/.backups/`. Excludes regenerable / cruft / secret paths (`logs/`, `integrations-cache/`, `.backups/`, `.DS_Store`, `.env*`). Used by the `data-backup` module's `backup-to-drive` skill; also whitelisted for direct `run_script` invocation. Tests: `tests/test_zip_data.py`.
