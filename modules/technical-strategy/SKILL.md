...
- **`cto-os-data/modules/technical-strategy/_module.md`** — singleton module activation record, overwrite-in-place. Activation step 3 adds or updates `adr_config` and records step 3 in `activation_completed`; all baseline `_module` frontmatter (`type`, `slug`, `module`, `updated`, `schema_version`, `active`, `activated_at`, `deactivated_at`) remains intact.
