...
- **`validate_state.py`**: exit 0 = clean, exit 1 = validation findings, exit 2 = bad args or operational failure.
...
- **`validate_deps.py`** — walks `modules/*/SKILL.md`, fails closed on missing or malformed SKILL frontmatter, and builds the required-dep graph to detect cycles or unknown required deps. Called by `hooks/pre-commit` whenever any module `SKILL.md` is staged. Also runnable on demand for debugging the graph.
- **`validate_state.py`** — read-only validator for `modules/{slug}/_module.md`, `modules/{slug}/state/**/*.md`, and `notes/**/*.md`. It checks readable mapping frontmatter, baseline fields and types, known schema types, `_module` structure, plus diagnosed checks for `requisition.opened` and `stakeholder-profile.tenure_months`; it deliberately is not a complete per-type schema engine. High-sensitivity content is skipped unless `include_high_sensitivity: true`.
